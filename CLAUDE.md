...
- **Step 5 (이슈 반응 예측 / Reaction Prediction) — 진행 중**: 이슈→차트 반응의 인과 패턴을 forward 로 축적하고, 새 이슈/종목에 대해 `방향·예상 변동폭 밴드·영향 기간·신뢰도·근거`를 출력하는 예측 레이어. 색깔 시스템(`이슈→종목 overall→포트폴리오 overall`)의 **예측 축 버전**. **핵심 철학(초기 설계)**: 파운데이션 모델(임베딩 검색 + LLM 서술)을 코어로 day-1 동작, **숫자는 실현 초과수익의 결정적 집계에서 나오고 LLM 은 근거 서술만**(환각 예측 방지·보정 가능). event_log 가 쌓일수록 개선. **(2026-06-09 현재)** 기본 엔진은 `gpt` 모드 — gpt-4o-mini 가 숫자까지 직접 출력. 8개 진단(아래)에서 임베딩·가격으로 방향·per-issue 변동폭 예측 신호 없음(NO-GO)이라 결정적 집계도 검증된 엣지가 없었기 때문. 원 철학의 결정적 집계 코어는 `model` 모드(k-NN)로 보존되며 `PREDICTION_MODE` 로 스위치. gpt 숫자는 검증된 예측이 아니라 판단.
  ...
  - **섀도 평가 하네스 + 사전등록 ✅ (2026-06-10)**: 사용자 결정 — 프로덕션은 **GPT-only 유지(아무것도 안 바꿈)**, 서버 오프라인에서 "통계가 GPT를 override할 자격이 되는가"를 **데이터 쌓이는 대로 재검증**하는 champion/challenger 모니터. 신규 `ml/SHADOW_EVAL.md`(사전등록 — 타깃·지표·baseline·세그·walk-forward·승격 게이트를 *반복 peek 전에* 못박아 p-hacking 차단) + `ml/shadow_eval.py`(스크리닝 러너, 결과 1행씩 `ml/shadow_log.jsonl` 적재 = 파생지표만, 커밋 가능 audit trail). 두 가설: **H1 within-stock 변동폭**(타깃=`|abret_h|` 을 **종목 내 train-only μ/σ로 표준화** → 자명한 cross-stock 스케일 제거, "이 이슈가 이 종목 평소보다 크냐"만; feat=임베딩 vs category one-hot; 지표 Spearman ρ+부트스트랩 CI, gate CI-low>0.05) / **H2 방향**(타깃=`sign(abret_h)`; rule=model(emb+sector)·sentiment(호재→up)·earnings; dir_hit vs majority·0.50, gate CI-low>0.53). 세그=market별(`US` 현재 전량 / **`KR`=0 → "awaiting data"**, forward 한국어+실 signal/intensity 가 유일한 미검증 문이라 절대 US와 혼합 안 함). 승격은 단일 split 스크리닝이 아니라 **≥1500건+3-fold walk-forward+BH-FDR q0.10** 통과 시에만(문서 §gate). **1차 실행 결과(2026-06-10, US 10,111건)**: H1 ρ 0.02~0.06(1d emb 0.061 최고, CI-low 0.019<gate), H2 dir_hit 전부 0.48~0.51(<majority·<0.50대). 특히 **earnings sentiment 룰 1d=0.492 < majority 0.562** → "호재/실적 → 다음날 상승"이 *초과수익 방향* 엣지가 아님을 직접 확인(사용자 직관의 정직한 반증). **verdict NO-GO — 프로덕션 GPT-only 유지.** KR 데이터 축적 시 자동 재검증, 십중팔구 NO-GO 유지가 예상 — 정직한 음성결과 추적 자체가 산출물. `data/`·`artifacts/` gitignore라 `.npz` 미커밋, 하네스 3파일은 커밋 가능. `.venv/bin/python ml/shadow_eval.py` ✅. **자동화 (2026-06-10)**: 신규 `.github/workflows/shadow-eval.yml` 이 **월 1회(매월 1일 06:00 UTC) GitHub Actions** 로 전체 파이프라인(pull_data → gpt_sentiment → shadow_eval) 실행 후 `shadow_log.jsonl` 자동 커밋 — **GitHub 클라우드라 Codespace stop/삭제와 무관**(데이터 적재도 Vercel cron→Supabase라 Codespace 독립). CI 안정화로 `shadow_eval.py` 2가지 수정: 미사용 `price_features.npz` 의존 제거(Yahoo fetch 불필요), `gpt_sentcat.npz` 없으면(OpenAI 키 누락/실패 시 `continue-on-error`) H1-emb·H2-model 만 graceful 실행. **사용자 작업**: repo Settings → Secrets(Actions)에 `SUPABASE_SECRET_KEY`·`OPENAI_API_KEY` 추가(`NEXT_PUBLIC_SUPABASE_URL` 은 keepalive 워크플로용으로 이미 등록됨). main 브랜치 보호 시 github-actions 봇 push 허용 필요. 결과 테이블은 Actions run summary 에도 출력. **첫 CI 실행 + 견고화 (2026-06-10)**: 첫 워크플로 실행(수동 dispatch, bot 자동 커밋 push 성공=브랜치 보호 무이슈)에서 H1-1d-emb 이 CI-low 0.0502 로 0.05 게이트를 간발로 넘어 "PROMOTE candidate" 로 떴음 — 단일 split·단일 seed MLP 의 **부동소수/플랫폼 경계 깜빡임**(로컬 동일 seed 는 CI-low 0.019 로 FAIL). 사용자 요청으로 `shadow_eval.py` 를 **2-stage 로 견고화**: (1) 스크리닝 메트릭을 **3-seed 앙상블**(val 예측 평균)로 → H1-1d-emb CI-low 0.028<0.05 로 정상 탈락, (2) **walk-forward 게이트** 신설(3 expanding fold, 각 fold 재표준화·재-within-stock μ/σ, fold CI-low 전부 게이트 초과 시 survive) + **BH-FDR q0.10** + 승격은 **≥2 horizon 동시 통과** 시에만(1d-only·3/5d 소멸 차단). `train_cell`/`score_seg` 분리로 (hyp,tag,horizon)당 1회 학습→seg 슬라이스 채점. **확인**: H1:emb 를 강제로 WF 에 넣어도 1d [-0.022,-0.011,0.049]/3d [0.011,-0.046,0.028]/5d [-0.034,-0.035,0.005] 전 fold 탈락 → 앙상블 스크리닝·WF 양쪽 독립 NO-GO. 교훈: 단일 split 스크리닝의 boundary PASS 는 노이즈 — 앙상블+WF+≥2horizon 이 가짜경보 차단. `verdict NO-GO` 유지.

- **Step 6 (모바일 기능 패리티) — 진행 중 (6-1 완료)**: 데스크탑에 추가된 기능 전부를 **모바일(<768)에만** 이식 (사용자 결정: 태블릿 768–1279 는 이번 범위 제외, 기능 공백 유지). 모바일 Figma 링크 7개는 `docs/design/mobile-figma-links.md` 에 저장 — **token limit 방지를 위해 작업하는 sub-task 의 노드만 그때그때 MCP 조회** (한 번에 전부 불러오지 말 것). **사용자 지시**: 7단계 색상 시스템·종목명은 Figma 가 아닌 현재 코드 로직을 따름. 임의 판단 금지, 애매하면 즉시 질문.
  - **현재 모바일 구현 상태 (2026-06-10 코드 실사)**: 메인 화면은 완성 — 2×4 카드그리드(150×160), 5×2 이슈그리드(topByPop 10), `StockNameMarquee`, sticky `TopTicker`(+`ArticleLink` 기사링크), 터치 3초 툴팁, `AnalyzingText`, 분석/캐시/Realtime/예측캐시 등 데이터 레이어 전부(뷰포트 무관). **공백 = 데스크탑 전용 UI 5종**이 `<1280` 에서 `display:none`: ① `AuthHeader`(→ 모바일에서 로그인 자체 불가) ② `NicknameModal` ③ `StockModal`(이슈 타임라인+예측 블록+backdrop — 카드 탭 시 `openStock` 은 불리지만 모달이 숨겨져 무동작으로 보임) ④ `EditPortfolioModal`(+온보딩 모드 — canEdit 사용자가 [수정] 탭해도 무동작) ⑤ `StockSearchDropdown`(모바일은 별도 "종목 검색 modal" 디자인 193:1296). 비로그인 [수정] 툴팁은 hover 전용이라 터치 무반응.
  - **Step 6 Decisions log (사용자 확정)**: 범위=모바일만(<768), 태블릿은 기존대로 / 비로그인 모바일 [수정] 탭 = **3초 툴팁**(기존 `ActiveTooltipContext` 터치 패턴 재사용, 같은 문구) / 색상·종목명은 코드 로직 우선(Figma 무시).
  - **구현 전략**: 로직(컨텍스트·Supabase·분석·예측)은 전부 재사용, 이번 작업은 **UI 레이어만**. 기존 컴포넌트에 모바일 분기/미디어쿼리 확장 우선 — `display:none` 가드를 `<1280` → **태블릿 구간만**(`768–1279`) 으로 좁히고 `<768` 모바일 레이아웃 추가. 레이아웃이 근본적으로 다른 것(종목 검색 모달)만 신규 컴포넌트. 모바일 frame 은 375px — 좌표는 전부 Figma MCP 값, 임의 수치 금지. 새 수치는 `design-tokens.md` **§16 (Mobile parity)** 신설해 기록.
  - **Sub-tasks (순서)**: **6-1** AuthHeader 모바일 (58:3 before-login + 76:1284 after-login 헤더부) + 비로그인 [수정] 탭 3초 툴팁 → **6-2** NicknameModal (187:5) → **6-3** StockModal (187:401 — 타임라인+예측 블록+backdrop, 터치 외부탭 닫기 검증) → **6-4** EditPortfolioModal 정적 UI+DnD (192:389 — dnd-kit 은 PointerSensor+touch-action:none 이라 터치 동작 예상, 실기기 검증 필요) → **6-5** 종목 검색 모달 (193:1296, 신규 컴포넌트 가능성 높음) + 편집 로직 연결 + 온보딩 모드 (192:2) → **6-6** 폴리시·회귀 (z-index, 외부탭/ESC 일관성, **데스크탑·태블릿 회귀 0** 확인). 각 sub-task: 해당 Figma 노드만 MCP 조회 → §16 기록 → 구현 → `npm run build` → push → 사용자 확인.
  - **6-1 ✅ (2026-06-10)**: AuthHeader 모바일. Figma 58:3(로그아웃)·76:1284(로그인) 메타데이터 + 헤더 노드 4개(76:131/1671/1672/1674) design context 직접 검증, 수치는 §16-1 기록. **CSS-only 모바일 레이아웃** (`AuthHeader.module.css`): 기존 `<1280 display:none` 가드를 **태블릿 구간만**(`768–1279`)으로 좁히고 `<768` override 추가 — 로그아웃 = "로그인 with"(Bold 8px/" with" 6px, 305·62) + Google 30×30(300·69) + Kakao 20×20(330·73), 로그인 = 닉네임(SemiBold 9px `#e5e5e5`, 148·70, 현재 포트폴리오 타이틀 위) + 로그아웃 버튼(57×26, 293·71, 12px). **모바일은 우상단 user 슬롯 없음**(`.userName` display:none — 닉네임 1곳만, Figma 의도). **current [수정] 모바일 x 275→251** (두 프레임 공통, 로그인 헤더 공간 — §13-3 정정, spare 275 유지). **비로그인 [수정] 터치 툴팁**: `EditButton` 이 ColorBox 패턴 그대로 `ActiveTooltipContext` 사용 (`pointerType==="touch"` 탭 = 3초 표시/재탭 닫기/다른 대상 탭 전환, hover 는 기존 CSS 그대로) + `.tooltipOpen::after` CSS 추가. OAuth/로그아웃 로직·DOM 구조 변경 0 (데스크탑 회귀 0). **알려진 중간상태**: 모바일에서 로그인은 되지만 닉네임 모달(6-2)·수정 모달(6-4)이 아직 숨겨져 있어 신규 가입 흐름은 6-2 이후 완성. 툴팁 폭(16px 폰트)이 375 프레임 우측을 약간 넘칠 수 있음 — ColorBox 모바일과 동일한 기존 동작이라 유지(Figma 모바일 툴팁 스펙 없음). `npm run build` ✅.
